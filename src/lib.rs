...
impl Vxls {
    pub const fn new(x_size: i32, y_size: i32, z_size: i32) -> Self {
        Self { x_range: 0..x_size, y_range: 0..y_size, z_range: 0..z_size }
    }
...
impl Skin {
    ...
    fn same_refrac(&self, dir: &UnitVec, rng: &mut ThreadRng) -> UnitVec {
        ...
        let mut cos_theta = 1. / (2. * G)
            * ((1. - self.g_square) / (2. * G).mul_add(rng.random::<f64>(), 1. - G))
                .mul_add(-((1. - self.g_square) / (2. * G).mul_add(rng.random::<f64>(), 1. - G)), 1. + self.g_square);
